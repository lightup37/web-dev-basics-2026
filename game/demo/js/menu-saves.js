...
(function () {
	...
	function starsText(s) {
		return s > 0 ? '★'.repeat(s) + '☆'.repeat(3 - s) : message('menu.cleared', null, '未通关');
	}
	...
	function routeCurve(a, b, index, fromNo, toNo) {
		var dx = b.x - a.x, dy = b.y - a.y;
		var length = Math.sqrt(dx * dx + dy * dy) || 1;
		var bend = Math.min(58, Math.max(20, length * 0.11)) * (index % 2 === 0 ? 1 : -1);
		/* 第 4→5 关（柏林方向→斯摩棱斯克）与随后折回的 5→6（滑铁卢）在第 5 关附近
	   弧段重合：4→5 强制做成向下（南）凸并加大弧度，与 5→6 的上凸分开。
	   y 轴向下为正，4→5 向东（dx>0），正 bend 即下凸。 */
		if (Number(fromNo) === 4 && Number(toNo) === 5) {
			bend = Math.abs(bend) * 1.45;
		}
	...
	function renderRoute(svg, entries, unlockId) {
		...
		for (var i = 0; i < entries.length - 1; i++) {
			...
			var d = routeCurve(a, b, i, entries[i].no, entries[i + 1].no);
	...
	function renderProgress() {
		...
		LEVELS.forEach(function (lv) {
			...
			// 已通关（有星级）一律显示红色 done；只有尚未通关且快照停在本关时才算“进行中”，
			// 避免读旧档残留的快照把已通关关卡显示成蓝色进行中标记。
			var cont = s === 0 && !!(auto.snapshot && Number(auto.snapshot.level) === lv.id);
			...
			entries.push({
				...
				title: localText(lv.name) + ' —— ' + (cont ? message('menu.pinContinue', null, '进行中，点标记继续')
					: (s > 0 ? mapStars(s) + message('menu.pinReplay', null, '，可重玩') : message('menu.pinStart', null, '未通关，点标记开始'))),
		...
		if (hm && hm.map && level6Cleared) {
			...
			entries.push({
				...
				title: routeOpen ? (localText(hm.name) + message('menu.hiddenSuffix', null, '（隐藏关）')) : lockedTitle,
	...
	function slotDetail(f) {
		if (!f) return message('save.empty', null, '（空）');
		var LEVELS = (typeof getLevelList === 'function') ? getLevelList() : [];
		var stars = f.stars || {};
		var beaten = [];
		var starSum = 0;
		LEVELS.forEach(function (lv) {
			...
			if (s > 0) {
				beaten.push(message('menu.levelStarsRow', { level: lv.id, stars: starsText(s) }, '第' + lv.id + '关 ' + starsText(s)));
				starSum += s;
			}
		});
		...
		parts.push(beaten.length
			? message('menu.slotCleared', { count: beaten.length, stars: starSum, total: (LEVELS.length * 3) }, '已通关 ' + beaten.length + ' 关 · 星级 ' + starSum + '/' + (LEVELS.length * 3))
			: message('menu.slotNoBeat', null, '尚无通关记录'));
		if (beaten.length) parts.push(beaten.join(' · '));
		parts.push(f.snapshot
			? message('menu.slotPlaying', { level: f.snapshot.level, turns: f.snapshot.remain_turns }, '第 ' + f.snapshot.level + ' 关进行中（剩余 ' + f.snapshot.remain_turns + ' 回合）')
			: message('menu.slotNoProgress', null, '无进行中的关'));
		return parts.join(' ｜ ');
	}
	...
	function renderManualList() {
		...
		manualSummaries(user).forEach(function (m) {
			...
			if (f) {
				...
				load.textContent = message('menu.load', null, '载入');
				load.onclick = (function (id) {
					return function () {
						var ask = message('menu.loadConfirm', { file: fileName(id) }, '载入 ' + fileName(id) + ' 会用它的内容覆盖当前自动存档 a.save，继续？');
						var doLoad = function () {
							if (loadManualToAuto(user, id)) {
								if (typeof toast === 'function') toast(message('menu.loaded', { file: fileName(id) }, '已载入 ' + fileName(id) + ' 到 a.save'));
								renderMenuSaves();
							}
						};
				...
				del.textContent = message('menu.delete', null, '删除');
				del.onclick = (function (id) {
					return function () {
						var ask = message('menu.deleteConfirm', { file: fileName(id) }, '删除 ' + fileName(id) + '？');
				...
				emptyLoad.textContent = message('menu.load', null, '载入');
				emptyLoad.onclick = (function (id, btn) {
					return function () {
						if (btn.dataset.armed !== '1') {
							btn.dataset.armed = '1';
							btn.textContent = message('menu.loadStill', null, '仍要载入');
							var hint = message('menu.emptySlotWarn', { file: fileName(id) }, fileName(id) + ' 是空的。再点一次将用它覆盖 a.save（清空进度，成就与手动备份保留）。');
							if (typeof toast === 'function') toast(hint); else alert(hint);
							return;
						}
						if (typeof clearAuto === 'function') clearAuto(user);
						if (typeof toast === 'function') toast(message('menu.emptyOverwrote', null, '已用空白存档覆盖 a.save：通关进度已清空'));
						renderMenuSaves();
					};
				})(m.id, emptyLoad);
	...
	function buildSaveModal() {
		...
		card.setAttribute('aria-label', message('save.modalTitle', null, '存档 / 读档'));

		var title = document.createElement('h3');
		title.className = 'save-modal-title';
		title.textContent = message('save.modalTitle', null, '存档 / 读档');
		...
		restart.title = message('menu.restartTitle', null, '清空活动存档 a.save，从头开始');
		restart.textContent = message('menu.restart', null, '重新开始游戏');
		...
		close.textContent = message('common.close', null, '关闭');
	...
	function bindMenuRestart() {
		...
		btn.addEventListener('click', function () {
			var user = currentU();
			if (!user) return;
			var ask = message('menu.restartConfirmFull', null, '重新开始将清空活动存档 a.save（通关进度 / 星级 / 进行中快照）与全部成就、连败记录，且不可恢复；手动备份 存档1~3 会保留。确定继续？');
			var doReset = function () {
				resetAutoSave(user);
				if (typeof toast === 'function') toast(message('menu.restartedFull', null, '已重新开始：a.save、成就与连败记录已清空'));
				renderMenuSaves();
